...
public:
  ...
  virtual void registerMatcherWithFinder(clang::ast_matchers::MatchFinder& Finder, clang::ast_matchers::MatchFinder::MatchCallback *Action) const override {
  ...
  virtual void processMatchResult(
    ...
    const ClangRulesetsCheckerReportDiagnostics& ReportDiagnostics) const override {
...
private:
  ...
  /// checker.
  void visitRecord(ASTContext &AST, const ClangRulesetsCheckerReportDiagnostics& ReportDiagnostics, const RecordDecl *RD, uint64_t PadMultiplier = 1) const {
    ...
    if (auto *CXXRD = dyn_cast<CXXRecordDecl>(RD))
      if (CXXRD->field_empty() && CXXRD->getNumBases() == 1)
        return visitRecord(AST, ReportDiagnostics, CXXRD->bases().begin()->getType()->getAsRecordDecl(),
                           PadMultiplier);
    ...
    reportRecord(AST, ReportDiagnostics, RD, BaselinePad, OptimalPad, OptimalFieldsOrder);
  }

  /// Look for arrays of overly padded types. If the padding of the
  /// array type exceeds AllowedPad, then generate a report.
  void visitVariable(ASTContext &AST, const ClangRulesetsCheckerReportDiagnostics& ReportDiagnostics, const VarDecl *VD) const {
  ...
  void reportRecord(
      ASTContext &AST, const ClangRulesetsCheckerReportDiagnostics& ReportDiagnostics, const RecordDecl *RD, CharUnits BaselinePad, CharUnits OptimalPad,
    ...
    ReportDiagnostics(std::vector<ClangRulesetsCheckerDiagnostic> {
      ClangRulesetsCheckerDiagnostic(RD->getSourceRange().getBegin(), false, Os.str())
    });
  }
};
